...
class PrivateRecipeTests(TestCase):
    ...
    def test_create_recipe_with_existing_ingredient(self):
        """Test creating a recipe with existing ingredient is successful."""
    ...
    def test_create_ingredients_on_recipe_update(self):
        ...
        recipe = create_recipe(user=self.user)
        url = detail_url(recipe.id)
        payload = {'ingredients': [{'name': 'Garlic'}, {'name': 'Salt'}]}
        res = self.client.patch(url, payload, format='json')
    ...
    def test_update_recipe_assign_ingredient(self):
        ...
        recipe = create_recipe(user=self.user)
        basic_ingredient = Ingredient.objects.create(
            user=self.user, name='Garlic')
        recipe.ingredients.add(basic_ingredient)
    ...
    def test_clear_recipe_ingredients(self):
        ...
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertIn(ingredient, Ingredient.objects.filter(user=self.user))
        recipe.refresh_from_db()
    ...
    def test_filter_recipe_by_tag(self):
        ...
        r1.tags.add(tag1)
        r2.tags.add(tag2)
        r3 = create_recipe(user=self.user, title='Fish soup')
    ...
    def test_filter_recipe_by_ingredients(self):
        """Test filter recipes by ingredients is successful"""
        i1 = Ingredient.objects.create(user=self.user, name='Solt')
        i2 = Ingredient.objects.create(user=self.user, name='Garlic')
        r1 = create_recipe(user=self.user, title='Borsch')
        r2 = create_recipe(user=self.user, title='Soup')
        r3 = create_recipe(user=self.user, title='Fry potato')
...
class ImageUploadCase(TestCase):
    ...
    def test_upload_image(self):
        """Test uploading an image toa recipe."""
